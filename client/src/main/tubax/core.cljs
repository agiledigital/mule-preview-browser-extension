...
; Forked from https://github.com/funcool/tubax to add line/column/position to each node
(defn- add-node-document
  ...
  (let [keytag (keyword (.-name node))
        ...
        node-value {:tag keytag
                    :attributes att-map
                    :content []
                    :location {:line (.-line parser)
  ...
  ([source {:keys [strict trim normalize
   ...
   (let [parser (.parser sax strict #js {"trim" trim
     ...
     (set! (.-onopentag parser)
           #(swap! document (partial add-node-document % parser)))
